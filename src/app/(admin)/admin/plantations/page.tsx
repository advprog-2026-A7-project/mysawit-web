          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {visiblePlantations.map(p => (
                <div key={p.id} className="glass-card surface-panel p-6 hover:border-green-500/20 transition-all">
                  ...
                  <div className="flex flex-col gap-3 pt-5 mt-3 border-t border-white/[0.06]">
                    <div className="flex gap-3">
                      <button onClick={() => openEdit(p)} className="flex-1 btn-secondary text-xs py-2 justify-center"><Pencil size={13} aria-hidden="true" />Edit</button>
                      {p.mandorId && (
                        <button onClick={() => handleUnassignMandor(p.id, p.name)} className="flex-1 btn-ghost text-xs py-2 justify-center text-yellow-400 border border-yellow-500/20 hover:bg-yellow-500/10">
                          <UserMinus size={13} aria-hidden="true" />Copot Mandor
                        </button>
                      )}
                    </div>
                    <button onClick={() => handleDelete(p.id, p.name)} className="w-full btn-danger text-xs py-2 justify-center bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20">
