...
const MANDOR_NAV_ITEMS = [
  { href: '/mandor/plantations', label: 'Kebun' },
  { href: '/mandor/harvest', label: 'Panen' },
  { href: '/mandor/shipment', label: 'Pengiriman' },
  { href: '/mandor/payroll', label: 'Validasi Gaji' },
];
...
export default function MandorLayout({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (item: typeof MANDOR_NAV_ITEMS[0]) => pathname.startsWith(item.href);

  if (!authService.isAuthenticated() || userInfo?.role !== 'MANDOR') return null;
    ...
    <div className="ms-layout">
      <header className="ms-navbar z-50">
        <div className="ms-navbar-inner max-w-7xl mx-auto w-full px-4 md:px-6 flex items-center justify-between h-16">
          <Link href="/mandor/plantations" className="flex items-center gap-2" onClick={() => setMenuOpen(false)}>
            <span className="bg-green-600 text-white p-1.5 rounded-md flex items-center justify-center">
              <Sprout size={18} />
            </span>
            <span className="font-bold text-white tracking-tight hidden sm:block">MySawit<span className="text-green-400 font-normal"> Mandor</span></span>
          </Link>

          <nav className="hidden md:flex items-center gap-6">
            {MANDOR_NAV_ITEMS.map((item) => {
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-[#0a1614] border border-[#152e2a] rounded-full">
              ...
              <span className="text-sm font-medium text-slate-200 truncate max-w-[100px]">{userInfo?.username || 'Mandor'}</span>
            </div>
            <button
